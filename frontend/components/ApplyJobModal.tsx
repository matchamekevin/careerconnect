    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.whatsappLinks && data.whatsappLinks.studentToCompany) {
          ...
          setMessage('✅ Candidature envoyée ! Ouverture de WhatsApp...');
          ...
          setTimeout(async () => {
            try {
              ...
              if (phoneNumber && message) {
                const result = await openWhatsApp(phoneNumber, decodeURIComponent(message));
                setMessage(`✅ Candidature envoyée ! WhatsApp ouvert ${result.method === 'app' ? 'via l\'application' : 'via le navigateur'}.`);
              } else {
                // Fallback vers l'ancienne méthode
                window.open(data.whatsappLinks.studentToCompany, '_blank');
                setMessage('✅ Candidature envoyée ! WhatsApp ouvert pour communication directe.');
              }
            } catch (error) {
        ...
        setMessage(data.error || 'Erreur lors de l\'envoi de la candidature');
      }
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-8 relative" onClick={(e) => e.stopPropagation()}>
        ...
        {message && (
          <div className={`mb-4 p-3 rounded-lg text-center ${message.includes('✅')
